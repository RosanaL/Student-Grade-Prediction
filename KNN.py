...
model = KNeighborsClassifier(n_neighbors=5)  # 你可以调整 n_neighbors 的值
...
if recall >= recall_threshold and specificity >= specificity_threshold and precision >= precision_threshold:
    # 输出数据到新的xlsx文件
    output_file_path = '/mnt/student_analysis/KNN_balanced_dataset.xlsx'
    ...
    plt.savefig('/mnt/student_analysis/KNN_confusion_matrix.png')
    plt.close()
else:
