...
# 转换为 PyTorch 张量
X_train_tensor = torch.tensor(X_train_scaled, dtype=torch.float32).unsqueeze(1)  # 添加一个额外的维度以适配 LSTM 输入
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            ...
            if recall >= recall_threshold and specificity >= specificity_threshold and precision >= precision_threshold:
                ...
                # 输出数据到新的xlsx文件
                output_file_path = '/mnt/student_analysis/balanced_dataset.xlsx'
                ...
                plt.savefig(f'/mnt/student_analysis/confusion_matrix_epoch_{epoch + 1}.png')
                plt.close()
                break
